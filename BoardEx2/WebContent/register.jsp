		...
		<script>
			...
			$(function(){
				$('input[name=id]').keyup(function(){
					var tag = $(this)
					var uid = tag.val();
					$.getJSON(path+'/member/validate.do?type=uid&param='+uid, function(data){
				...
				$('input[name=nick]').keyup(function(){
					var tag = $(this)
					var uid = tag.val();
					$.getJSON(path+'/member/validate.do?type=nick&param='+uid, function(data){
				...
				$('input[name=email]').keyup(function(){
					var tag = $(this)
					var uid = tag.val();
					$.getJSON(path+'/member/validate.do?type=email&param='+uid, function(data){
				...
				$('input[name=hp]').keyup(function(){
					var tag = $(this)
					var uid = tag.val();
					$.getJSON(path+'/member/validate.do?type=hp&param='+uid, function(data){
						$(".resultHp").css("color",data.color).text(data.result);
						if(data.color != "red") {
							isNickOk = 1;
						} else {
							isNickOk = 0;
	...
	<body>
		<div id="member">
			<section class="register">
				<form action="${ cPath }/member/register.do" method="POST">
					<section>
						<table>
					...
					<div>
						<a href="${ cPath }/member/login.do" class="cancel">취소</a> 
